                      ...
                      <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                        ...
                        <div class="cart-foot-r">
                            <div class="item-total">
                                商品总计: <span class="total-price">{{ totalMoney }}</span>
                            </div>
...
export default {
  ...
  mounted() {   //将模板挂在到页面上之后执行该函数，为了确保视图都渲染完毕，
                //再调用方法，用了nextTick
  ...
  methods: {
      cartView() {
          this.cartList = this.$store.state.cartList;  //从状态中取购物车数据
      },
      changeMoney(product, way) {  //改变价格
          if (way > 0) {
      ...
      selectedProduct(item) {     //先在句柄上绑定一个属性，然后用方法改变这个属性，选择商品，如果商品的数据的选项是未定义，就增加一个属性checked，值为true
      ...
      checkAll(flag) {        //全选
          this.checkAllFlag = flag;   //标志位参数传值是true，再点击是
      ...
      delProduct() {   //删除商品
          this.cartList.splice(this.deleteIndex, 1);  //splice方法，获取当前商品下标，删除长度为1
          this.delFlag = false;   //删除标志设为false
          this.$store.commit("saveCartList",this.cartList);  //用commit唤醒事件，把当前购物列表状态放到管理状态中
      }
  },
  computed : {   //计算总金额，因为是价格是实时改变的，所以要写到computed中
    totalMoney () {  //总价格等于单价*数量
        return this.cartList.reduce((total, item) => {//遍历商品列表，如果商品被选择，那他的价格会被累加到总价中
