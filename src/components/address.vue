...
<script>
  ...
  data () {
    ...
    computed: {   //过滤地址，设置显示数量
        filterAddress() {
            return this.addressList.slice(0, this.limitNum);
        }
    },
    methods: {   
        getAddressList() {   //获取地址列表,使用vue-source方法，获取本地数据
            this.$http.get("/src/assets/data/address.json").then((response)=> {
        ...
        loadMore() {     //显示更多地址，改变限制长度，用computed计算
            this.limitNum = this.addressList.length;
        },
        setDefault(addressId) {   //设为默认地址,遍历地址列表后，如果当前地址的id和点击的id相同，就将默认属性设为true，用v-if展示
